...
export function useVoiceChat(otherParticipantId) {
  ...
  const connect = async () => {
    ...
    try {
      ...
      console.log('Got local microphone stream:', localStreamRef.current.getTracks())
      ...
      console.log('Connected local stream to analyzer')
      ...
      console.log('WebRTC connection created', { pc, dc })
      pcRef.current = pc
      dcRef.current = dc
  ...
  const disconnect = useCallback(() => {
    console.log('Voice Chat: Disconnecting...')
  ...
  const handleServerEvent = (event) => {
    console.log('Voice Chat: Received server event:', event)
